...
public class HydrationListener implements TemperatureListener {
    ...
    public void temperatureEvent(TemperatureEvent event) {
        ...
        final float hydration = persistentDataContainer.getOrDefault(Constants.HYDRATION_KEY, PersistentDataType.FLOAT, maxHydration) - hydrationChange;
        ...
        player.spigot().sendMessage(ChatMessageType.ACTION_BAR, new TextComponent("Hydration: " + Constants.NUMBER_FORMAT.format(hydration) + "%" + " Temperature: " + Constants.NUMBER_FORMAT.format(temperature) + "°C"));
